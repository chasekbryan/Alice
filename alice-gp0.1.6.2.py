...
# ---
# Alice gp0.1.6 — NOAA weather, web access toggle, package install, pentest (detection-only), performance tweaks
# New in p0.1.5:
...
class Config:
    ...
    ghidra_headless: str = ""  # optional path to analyzeHeadless
...
class AliceHeadless(GhidraScript):
    def _args_map(self):
        out = {}
        a = self.getScriptArgs()
        if a:
            for s in a:
                s = str(s)
                if "=" in s:
    ...
    def _json(self, obj):
        try:
            ...
            return '{"ok":false,"error":"json-dump-failed"}'

    def _find_function(self, prog, name, addr_str):
    ...
    def run(self):
        ...
        if mode == "functions":
            ...
            while it.hasNext() and not self.monitor.isCancelled():
                ...
                try:
                    ...
                    funs.append({"name": f.getName(), "addr": str(entry), "size": int(size)})
                except:
                    pass
            self._write(out, self._json({"ok": True, "kind":"functions", "count": len(funs), "functions": funs}))
            return

        if mode == "decompile":
            target_name = args.get("func","")
            addr_str = args.get("addr","")
            f = self._find_function(prog, target_name, addr_str)
            if not f:
                self._write(out, self._json({"ok": False, "error": "function-not-found"}))
                return
            iface = DecompInterface()
            try:
                iface.openProgram(prog)
                res = iface.decompileFunction(f, 60, self.monitor)
                if res and res.getDecompiledFunction():
                    code = res.getDecompiledFunction().getC()
                    self._write(out, code)
                    return
                else:
                    self._write(out, self._json({"ok": False, "error": "decompiler-failed"}))
                    return
            finally:
                try:
                    iface.dispose()
                    ...
                    try:
                        ...
                        for i in range(ins.getNumOperands()):
                            try:
                                ...
                                pass
                        line = "%s: %s %s" % (str(addr), mnem, ", ".join(ops) if ops else "")
                        buf.append(line)
                    except:
                        pass

            if target_name or addr_str:
                f = self._find_function(prog, target_name, addr_str)
                if f:
                ...
                try:
                    af = prog.getAddressFactory()
                    a0 = af.getAddress(start)
                    a1 = af.getAddress(end)
                    aset = AddressSet(a0, a1)
                    it = listing.getInstructions(aset, True)
                    dump_iter(it)
                except:
                    self._write(out, self._json({"ok": False, "error": "bad-range"}))
                    return
            else:
                self._write(out, self._json({"ok": False, "error": "missing-target"}))
                return
        ...
        self._write(out, self._json({"ok": False, "error": "unknown-mode"}))
"""
# ----------------------- ALICE CORE -----------------------
class Alice:
    def __init__(self, cfg: Config):
    ...
    def _run_tool(self, name: str, args: Dict[str, Any]) -> str:
        builtins = {
                        "ghidra_functions": self._tool_ghidra_functions,
            "ghidra_decompile": self._tool_ghidra_decompile,
            "ghidra_disasm": self._tool_ghidra_disasm,
"calc": self._tool_calc,
    ...
    def _help_text(self) -> str:
        ...
        - Ghidra (headless, local-only)
        - /run ghidra_functions {"file":"/path/to/bin"}
        - /run ghidra_decompile {"file":"/path/to/bin","function":"main"}
        - /run ghidra_disasm {"file":"/path/to/bin","function":"main"}
    ...
    def _cmd_run(self, raw: str) -> str:
        ...
        if name in ("calc", "recall", "set_fact", "get_facts", "browse", "noaa_weather",
                    "nmap_scan", "nikto_scan", "zap_scan", "nuclei_scan", "msf_check", "wifi_scan", "ghidra_functions", "ghidra_decompile", "ghidra_disasm"):
...
# ---------- evidence helper (general) ----------
def _save_evidence(self, name_hint: str, content: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "_", (name_hint or "out"))[:64]
    ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs("evidence", exist_ok=True)
    path = os.path.join("evidence", f"{ts}_{safe}")
    ...
    def _find_ghidra_headless(self) -> Optional[str]:
        cand = (self.cfg.ghidra_headless or os.environ.get("GHIDRA_HEADLESS") or "").strip()
        if cand and os.path.exists(cand):
            return cand
        for nm in ("analyzeHeadless", "analyzeHeadless.bat"):
            pth = shutil.which(nm)
            if pth:
                return pth
        base = os.environ.get("GHIDRA_INSTALL", "").strip()
        if base:
            for nm in ("analyzeHeadless", "analyzeHeadless.bat"):
                pth = os.path.join(base, "support", nm)
                if os.path.exists(pth):
                    return pth
        return None

    def _ghidra_write_script(self, dirpath: str) -> str:
        script_path = os.path.join(dirpath, "alice_ghidra_extract.py")
        with open(script_path, "w", encoding="utf-8") as f:
            f.write(_ALICE_GHIDRA_JYTHON)
        return script_path

    def _ghidra_run(self, file_path: str, mode: str, **kwargs) -> Dict[str, Any]:
        headless = self._find_ghidra_headless()
        if not headless:
            return {"ok": False, "error": "Ghidra analyzeHeadless not found. Set GHIDRA_HEADLESS or add to PATH."}
        file_path = os.path.abspath(os.path.expanduser(file_path))
        if not os.path.exists(file_path):
            return {"ok": False, "error": f"file not found: {file_path}"}
        import tempfile, subprocess, json as _json
        tmpdir = tempfile.mkdtemp(prefix="alice_ghidra_")
        out_suffix = {"functions": ".json", "decompile": ".c", "disasm": ".asm"}.get(mode, ".out")
        out_file = os.path.join(tmpdir, "alice_out" + out_suffix)
        script_file = self._ghidra_write_script(tmpdir)
        projdir = os.path.join(tmpdir, "proj")
        os.makedirs(projdir, exist_ok=True)
        cmd = [headless, projdir, "AliceProj", "-import", file_path,
               "-scriptPath", tmpdir, "-postScript", os.path.basename(script_file),
               f"mode={mode}", f"out={out_file}", "-deleteProject"]
        for k in ("func","addr","start","end","function"):
            v = kwargs.get(k)
            if isinstance(v, str):
                v = v.strip()
            if v:
        ...
        try:
            ...
            if not os.path.exists(out_file):
                err_tail = ("\n".join((cp.stderr or cp.stdout or "").splitlines()[-20:]))
                return {"ok": False, "error": "ghidra headless failed", "detail": err_tail}
            if mode == "functions":
                try:
                    data = _json.load(open(out_file, "r"))
                except Exception as e:
                    raw = open(out_file, "r", encoding="utf-8", errors="ignore").read()
                    return {"ok": False, "error": "bad-json", "detail": str(e), "raw": raw[:1000]}
                return {"ok": True, "result": data}
            else:
                text = open(out_file, "r", encoding="utf-8", errors="ignore").read()
                ev_name = f"ghidra_{mode}_{os.path.basename(file_path)}{out_suffix}"
                ev_path = self._save_evidence(ev_name, text)
                preview = "\n".join(text.splitlines()[:60])
                return {"ok": True, "result": {"evidence": ev_path, "preview": preview}}
        except subprocess.TimeoutExpired:
            return {"ok": False, "error": "ghidra timeout"}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    # ---------- built-in ghidra tools ----------
    def _tool_ghidra_functions(self, args: Dict[str, Any]) -> Dict[str, Any]:
        file_path = str(args.get("file","")).strip()
        if not file_path:
            return {"ok": False, "error": "missing 'file'"}
        return self._ghidra_run(file_path, "functions")

    def _tool_ghidra_decompile(self, args: Dict[str, Any]) -> Dict[str, Any]:
        file_path = str(args.get("file","")).strip()
        func = str(args.get("function","") or args.get("func","")).strip()
        addr = str(args.get("addr","")).strip()
        if not file_path or not (func or addr):
            return {"ok": False, "error": "usage: file + (function or addr)"}
        return self._ghidra_run(file_path, "decompile", func=func, addr=addr)

    def _tool_ghidra_disasm(self, args: Dict[str, Any]) -> Dict[str, Any]:
        file_path = str(args.get("file","")).strip()
        func = str(args.get("function","") or args.get("func","")).strip()
        addr = str(args.get("addr","")).strip()
        ...
        if not file_path:
            return {"ok": False, "error": "missing 'file'"}
        if not (func or addr or (start and end)):
            return {"ok": False, "error": "usage: file + (function|addr|start+end)"}
        return self._ghidra_run(file_path, "disasm", func=func, addr=addr, start=start, end=end)
    # ---------- ghidra headless helpers ----------
    def _find_ghidra_headless(self) -> Optional[str]:
        cand = (self.cfg.ghidra_headless or os.environ.get("GHIDRA_HEADLESS") or "").strip()
        if cand and os.path.exists(cand):
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Alice gp0.1.6 — continuously-learning local AI (NOAA weather enabled)")
    ...
    print("Alice gp0.1.6.2 is awake. Type /help for commands. (Ctrl+C to exit.)")
