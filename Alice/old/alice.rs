...
impl Memory {
    ...
    fn recall(&self, query: &str, k: usize) -> Result<String> {
        ...
        let mut results: Vec<(i64, String)> = rows.collect::<Result<_, _>>()?;
        ...
        if results.is_empty() {
            ...
            let mut scored: Vec<(i64, String, usize)> = rows_fb.collect::<Result<_, _>>()?
                .into_iter()
                .map(|(id, content)| {
                    let score = query.split_whitespace().filter(|t| content.to_lowercase().contains(&t.to_lowercase())).count();
                    (id, content, score)
                })
                .filter(|(_, _, s)| *s > 0)
                .collect();
...
impl LlmClient {
    ...
    fn list_models(&self) -> Result<Vec<String>> {
        ...
        let models = resp["models"].as_array().unwrap_or(&vec![]);
...
impl Alice {
    ...
    fn build_history_context(&self) -> Result<String> {
        ...
        let mut hist: Vec<(String, String)> = rows.collect::<Result<_, _>>()?;
        hist.reverse();
    ...
    fn exec_tool(&mut self, name: &str, args: &HashMap<String, JsonValue>) -> Result<String> {
        match name {
            "calc" => {
                ...
                // Use evalexpr or simple calc; stub with rust eval
                let res = evalexpr::eval(expr).map(|v| v.to_string()).unwrap_or_else(|e| e.to_string());
                Ok(json!({"ok": true, "result": res}).to_string())
            }
            "recall" => {
                let query = args.get("query").and_then(|v| v.as_str()).unwrap_or("");
                self.mem.recall(query, self.cfg.args.recall_k as usize)
            }
            "set_fact" => {
...
- /doctor — diagnostics"#.to_string()
    ...
    fn cmd_run(&mut self, parts: &[&str]) -> Result<String> {
        if parts.len() < 2 {
            return Err(anyhow!("usage: /run <name> {json args}"));
        }
        let name = parts[1];
    ...
    fn train_snapshot(mem: &Memory, dir: &str) -> Result<()> {
        ...
        let mut user = None;
        for r in rows {
            let (role, content): (String, String) = r?;
            if role == "system" {
    ...
    fn cmd_weather(&mut self, parts: &[&str]) -> Result<String> {
        ...
        let lon: coords[1].parse().unwrap_or(0.0);
    ...
    fn tool_nikto_scan(&self, url: &str) -> Result<String> {
        let host = url.parse::<url::Url>().ok().and_then(|u| u.host_str().map(str::to_lowercase)).unwrap_or_default();
...
fn main() -> Result<()> {
    let args = Args::parse();
    let mut alice = Alice::new(Config { args })?;
